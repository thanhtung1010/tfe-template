...
- **Standalone Components**: Standarlone is default, do not need `@Component({standalone: true})`.
...
- **Selector Prefixing**:
    ...
    - Do not hardcode `tfe-` if the configuration specifies otherwise.
...
- **Style Management & SCSS Workflow**:
    ...
    - **Find & Match Logic**: Before creating a new style rule, locate the appropriate file in `src/styles/scss/`: - _Animations_: Update `animations.scss` for keyframes or complex transitions. - _Component-specific_: Update `components.scss` for reusable UI patterns. - _Selectors/Resets_: Update `selectors.scss` for tag-level styling. - _Variables_: Update `variable.scss` for colors/spacing.
      And do not create component-level.scss files under any circumstances.
